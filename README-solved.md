...
* Create a Pod with three busy box containers with the following commands respectively and check the status:
  * `"ls; sleep 3600;"`
...
### Lab-02
* Create a Pod with main container image "busybox" which executes the command below and with sidecar nginx container which exposes the container on port 80. Verify that both containers are running.
...
* Create 6 nginx pods in which two of them are labeled env=prod, another two are labeled env=acc, and the last two are labeled env=dev and verfiy that the pods are created with correct labels
